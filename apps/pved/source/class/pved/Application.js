 ...
 * @asset(pved/*)
 */
qx.Class.define("pved.Application", {
    extend: qx.application.Standalone,
    include: [
        pvec.application.MApplication,
    ],
    ...
    members: {
        ...
        main: function () {
            ...
            // Init pvec.application.MApplication
            this.initve("pved.page.");
            ...
            // Search Field
            var sr = new pved.part.SearchResources("cellTap", ["type", "description", "node", "pool"]);
            sr.setStateStoreKey("qx-globalsearchtable-columnstate");
            var srf = sr.getSearchField();
            var srTable = sr.getContainer();
            var searchWindow = new pved.window.SearchTable(srTable);
            ...
            var proxmoxLogo = new qx.ui.basic.Image("p/images/proxmox_logo.png");
            ...
            headerColumn.add(new p.ui.basic.CssImage(["fa", "fa-gear"]).set({ appearance: "header-label" }));
            var docButton = new p.ui.form.CssButton(this.tr("Documentation"), ["fa", "fa-book"]).set({ appearance: "white-button-header" });
            docButton.addListener("execute", (e) => {
                var win = window.open("/pve-docs/index.html", '_blank');
                win.focus();
            });
            headerColumn.add(docButton);
            headerColumn.add(new p.ui.form.CssButton(this.tr("Create VM"), ["fa", "fa-desktop"]).set({ appearance: "blue-button-header" }));
            var buttonCreateCT = new p.ui.form.CssButton(this.tr("Create CT"), ["fa", "fa-cube"]).set({ appearance: "blue-button-header" });
            headerColumn.add(buttonCreateCT);
            var logoutButton = new p.ui.form.CssButton(this.tr("Logout"), ["fa", "fa-sign-out"]).set({ appearance: "blue-button-header" });
            ...
            // Left tree column
            var serverBrowser = new pved.part.ServerBrowser();
        ...
        _onLogout: function(data) {
            ...
            this._loginWindow = new pved.window.Login();
